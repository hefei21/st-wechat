...
- 手动记忆 `/memory` 与 SillyTavern 内置“总结”扩展共享同一份总结，并自动注入上下文
- 扩展面板实时显示二维码和状态
- 容器重启自动恢复凭证，无需重复扫码
...
| 命令 | 说明 |
|------|------|
| `/memory [内容]` | 不带内容时读取酒馆当前总结，带内容时更新酒馆总结；设置前需至少完成一轮对话 |
...
| 世界书 | ✅ 自动匹配注入 | — |
| 总结记忆 | ✅ 读取 SillyTavern 总结扩展的 `extra.memory` | `/memory` 写回同一字段并刷新当前浏览器聊天 |
...
`/memory` 与 SillyTavern 内置“总结”扩展使用同一份聊天内总结：浏览器生成或编辑总结后，
Bot 会在读取状态和下一次生成前从聊天文件重新加载；Bot 更新总结后，如果浏览器正打开
同一聊天，会自动重载该 chatId 并显示新总结。旧版 ST WeChat 写在聊天首行的 `summary`
仍可只读恢复，但所有新写入均使用 SillyTavern 的 `extra.memory` 格式。该格式已验证兼容
SillyTavern 1.16.0；升级 SillyTavern 后建议按发布说明执行总结双向读取复验。
